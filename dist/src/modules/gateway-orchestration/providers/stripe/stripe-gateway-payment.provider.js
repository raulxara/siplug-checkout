...
exports.StripeGatewayPaymentProvider = void 0;
const common_1 = require("@nestjs/common");
const gateway_payment_dto_out_1 = require("../../dtos/gateway-payment.dto-out");
let StripeGatewayPaymentProvider = class StripeGatewayPaymentProvider {
    getProviderName() {
        return 'stripe';
    }
    ...
    async processCheckoutSession(dtoIn) {
        ...
        if (!response.ok) {
            return new gateway_payment_dto_out_1.GatewayPaymentDtoOut({
                ...
                processMessage: this.extractStripeErrorMessage(responseBody) ??
    ...
    buildCheckoutSessionRequestPayload(dtoIn) {
        ...
        const successUrl = this.toNullableString(transactionConfig.successUrl) ??
        ...
        const boletoExpiresAfterDays = this.toPositiveInteger(transactionConfig.boletoExpiresAfterDays) ??
        ...
        if (paymentMethodTypes.includes('boleto') &&
            ...
            payload['payment_method_options[boleto][expires_after_days]'] = String(boletoExpiresAfterDays);
        }
        return payload;
    }
    resolveStripePaymentMethodTypes(dtoIn) {
        const config = dtoIn.config ?? {};
        const transactionConfig = this.asObject(config.transactionConfig);
        const apiCredentialConfig = this.asObject(config.apiCredentialConfig);
        ...
        const configured = this.asStringArray(transactionConfig.stripePaymentMethods) ??
    ...
    buildStripeLineItems(dtoIn) {
        const providerPayload = dtoIn.providerPayload ?? {};
        const rawItems = providerPayload.items;
        const currency = String(dtoIn.paymentTransaction.currency ?? 'BRL').toLowerCase();
        ...
        return rawItems
            ...
            return {
                referenceId: this.toNullableString(item.itemRef) ??
    ...
    mapSuccessfulCheckoutSessionResponse(params) {
        const gatewayStatus = this.toNullableString(params.responseBody.payment_status) ??
        ...
        return new gateway_payment_dto_out_1.GatewayPaymentDtoOut({
            ...
            expiresAt: this.formatUnixTimestampToSqlDateTime(params.responseBody.expires_at) ??
                params.dtoIn.paymentTransaction.expiresAt,
        });
    }
    mapStripeStatusToInternalStatus(gatewayStatus) {
    ...
    mapStripeStatusToProcessStatus(gatewayStatus) {
        const status = this.normalize(gatewayStatus);
        if (['paid', 'complete', 'succeeded'].includes(status)) {
    ...
    extractStripeErrorMessage(responseBody) {
        return (this.toNullableString(responseBody.error?.message) ??
            this.toNullableString(responseBody.error?.code));
    }
    formatUnixTimestampToSqlDateTime(value) {
...
exports.StripeGatewayPaymentProvider = StripeGatewayPaymentProvider;
exports.StripeGatewayPaymentProvider = StripeGatewayPaymentProvider = __decorate([
    (0, common_1.Injectable)()
], StripeGatewayPaymentProvider);
//# sourceMappingURL=stripe-gateway-payment.provider.js.map