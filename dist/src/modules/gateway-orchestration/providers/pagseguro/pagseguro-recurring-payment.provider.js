...
let PagSeguroRecurringPaymentProvider = class PagSeguroRecurringPaymentProvider {
    ...
    buildNativeSplit(dtoIn) {
        ...
        if (recipients.length === 0) {
            throw new Error('PagSeguro native recurring split requires calculated payment split recipients');
        }
        ...
        if (calculationBase !== null && calculationBase !== 'gross_amount') {
            throw new Error('PagSeguro native recurring split requires a split rule calculated from gross_amount');
        }
        const mappedRecipients = recipients.map((recipient) => {
            ...
            const pagSeguroAccount = this.asObject(gatewayAccounts.pagseguro ?? gatewayAccounts.pagbank);
            ...
            if (accountId === null || !accountId.startsWith('ACCO_')) {
                throw new Error(`PagSeguro Account_ID is required for recurring split recipient ${splitRecipientId}`);
            }
            const amount = Number(recipient.amount);
            const percentage = this.toNullableNumber(recipient.percentage);
            if (!Number.isInteger(amount) || amount < 0) {
                throw new Error('PagSeguro recurring split recipient amount must be a non-negative integer in cents');
            }
        ...
        if (totalAmount !== dtoIn.paymentTransaction.amount) {
            throw new Error('PagSeguro FIXED recurring split recipients must allocate the full gross subscription amount');
        }
        return {
            method: 'FIXED',
            receivers: mappedRecipients.map((recipient) => ({
    ...
    assertSplitCompatibleSubscription(dtoIn) {
        ...
        const configuredCoupon = this.toNullableString(planConfig.couponId) ??
        ...
        const setupFee = this.toNullableNumber(planConfig.setupFee) ??
            this.toNullableNumber(planConfig.setup_fee);
        if (configuredCoupon !== null ||
            (Array.isArray(planConfig.coupons) && planConfig.coupons.length > 0)) {
            throw new Error('PagSeguro native recurring split cannot be combined with coupons');
        }
        if (setupFee !== null && setupFee > 0) {
            throw new Error('PagSeguro native recurring split cannot be combined with setup_fee');
        }
    }
