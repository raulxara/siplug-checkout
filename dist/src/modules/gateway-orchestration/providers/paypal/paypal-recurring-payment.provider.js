...
exports.PayPalRecurringPaymentProvider = void 0;
const common_1 = require("@nestjs/common");
const gateway_recurring_payment_dto_out_1 = require("../../dtos/gateway-recurring-payment.dto-out");
let PayPalRecurringPaymentProvider = class PayPalRecurringPaymentProvider {
    async createSubscription(dtoIn) {
        ...
        if (!tokenDtoOut.ok) {
            return this.buildFailedResponse({
                ...
                processMessage: this.resolvePayPalErrorMessage(tokenDtoOut.body, tokenDtoOut.status),
                providerRequest: {
        ...
        const accessToken = this.toRequiredString(tokenDtoOut.body.access_token, 'PayPal access_token was not returned');
        ...
        if (!productAndPlanDtoOut.ok) {
            return this.buildFailedResponse({
                ...
                processMessage: this.resolvePayPalErrorMessage(productAndPlanDtoOut.body, productAndPlanDtoOut.status),
        ...
        if (!subscriptionResponse.ok) {
            return this.buildFailedResponse({
                ...
                processMessage: this.resolvePayPalErrorMessage(subscriptionResponse.body, subscriptionResponse.status),
        ...
        return new gateway_recurring_payment_dto_out_1.GatewayRecurringPaymentDtoOut(true, 'paypal', gatewaySubscriptionId, planId, null, gatewaySubscriptionId, gatewayStatus, mappedStatus.status, mappedStatus.processStatus, mappedStatus.processMessage, this.sanitizePayload({
            ...
            planId,
        }, approvalUrl, approvalUrl, null, null, null, null, null, null, null, null, dtoIn.paymentTransaction.expiresAt);
    }
    async resolveOrCreatePlan(params) {
        ...
        const productId = this.toRequiredString(productResponse.body.id, 'PayPal product id was not returned');
        ...
        const planId = this.toRequiredString(planResponse.body.id, 'PayPal plan id was not returned');
        return {
            ok: true,
            status: planResponse.status,
            body: planResponse.body,
    ...
    buildProductRequest(dtoIn) {
        return {
            name: this.limitText(dtoIn.subscriptionPlan.name, 127),
            description: this.limitText(dtoIn.subscriptionPlan.description ??
                dtoIn.subscriptionPlan.name, 256),
            type: 'SERVICE',
            category: 'SOFTWARE',
    ...
    buildPlanRequest(params) {
        ...
        billingCycles.push({
            frequency: {
                interval_unit: this.mapPayPalInterval(dtoIn.subscriptionPlan.billingInterval),
        ...
        return {
            ...
            name: this.limitText(dtoIn.subscriptionPlan.name, 127),
            description: this.limitText(dtoIn.subscriptionPlan.description ??
                dtoIn.subscriptionPlan.name, 127),
            status: 'ACTIVE',
    ...
    resolvePayPalRedirectUrl(params) {
    ...
    async createAccessToken(dtoIn) {
        ...
        const body = await this.parseJsonResponse(response, 'PayPal returned a non JSON OAuth response');
        return {
            ok: response.ok,
    ...
    async executePayPalJsonRequest(params) {
        ...
        const body = await this.parseJsonResponse(response, `PayPal returned a non JSON response for ${params.path}`);
        return {
            ok: response.ok,
            status: response.status,
            body,
    ...
    resolvePayPalPlanId(dtoIn) {
        ...
        return (this.toNullableString(paypalMapping.planId) ??
    ...
    resolvePayPalProductId(dtoIn) {
        const planConfig = this.asObject(dtoIn.subscriptionPlan.config);
        const gatewayMappings = this.asObject(planConfig.gatewayMappings);
        const paypalMapping = this.asObject(gatewayMappings.paypal);
        return (this.toNullableString(paypalMapping.productId) ??
    ...
    mapPayPalSubscriptionStatus(status) {
        ...
        if (normalized === 'APPROVAL_PENDING') {
            return {
                ...
                processMessage: 'PayPal recurring subscription created and waiting buyer approval',
            };
        }
        if (normalized === 'APPROVED') {
            return {
                ...
                processMessage: 'PayPal recurring subscription approved and waiting activation',
            };
        }
    ...
    resolvePayPalErrorMessage(responseBody, statusCode) {
    ...
    resolveSuccessUrl(dtoIn) {
        ...
        const apiCredentialConfig = this.asObject(dtoIn.apiCredential.config);
        const gatewayConfig = this.asObject(dtoIn.config.gatewayConfig);
        return (this.toNullableString(checkoutSession.successUrl) ??
    ...
    resolveCancelUrl(dtoIn) {
        ...
        const apiCredentialConfig = this.asObject(dtoIn.apiCredential.config);
        const gatewayConfig = this.asObject(dtoIn.config.gatewayConfig);
        return (this.toNullableString(checkoutSession.cancelUrl) ??
...
exports.PayPalRecurringPaymentProvider = PayPalRecurringPaymentProvider;
exports.PayPalRecurringPaymentProvider = PayPalRecurringPaymentProvider = __decorate([
    (0, common_1.Injectable)()
], PayPalRecurringPaymentProvider);
//# sourceMappingURL=paypal-recurring-payment.provider.js.map