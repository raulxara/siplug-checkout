...
exports.NormalizeInfinitePayWebhookService = void 0;
...
const normalize_infinitepay_webhook_dto_out_1 = require("./dtos/normalize-infinitepay-webhook.dto-out");
let NormalizeInfinitePayWebhookService = class NormalizeInfinitePayWebhookService {
    exec(dtoIn) {
        const payload = this.resolvePayloadRoot(dtoIn.payload);
        const orderNsu = this.getString(payload, 'order_nsu') ??
        ...
        const transactionNsu = this.getString(payload, 'transaction_nsu') ??
        ...
        const invoiceSlug = this.getString(payload, 'invoice_slug') ??
        ...
        const fallbackId = this.getString(payload, 'id') ??
            this.getString(dtoIn.payload, 'id') ??
            this.getString(dtoIn.payload, 'event_id') ??
            this.getString(dtoIn.payload, 'eventId');
        const gatewayTransactionId = orderNsu ?? transactionNsu ?? invoiceSlug ?? fallbackId;
        if (gatewayTransactionId === null) {
            throw new Error(`InfinitePay webhook requires an identifiable order or transaction id. Payload: ${JSON.stringify(dtoIn.payload)}`);
        }
        const captureMethod = this.getString(payload, 'capture_method') ??
        ...
        const normalizedEvent = new normalized_payment_webhook_event_dto_1.NormalizedPaymentWebhookEventDto({
            ...
            eventId,
            eventType,
            eventAction: `payment.${captureMethod}.${this.resolveActionSuffix(payload)}`,
        ...
        return new normalize_infinitepay_webhook_dto_out_1.NormalizeInfinitePayWebhookDtoOut(normalizedEvent);
    }
    resolvePayloadRoot(payload) {
        const possibleRoots = [
    ...
    resolveCanonicalStatus(payload) {
        ...
        if (status !== null) {
            ...
            if (['paid', 'approved', 'completed', 'captured', 'confirmed', 'success'].includes(normalizedStatus)) {
                return 'paid';
            }
    ...
    resolveAmount(payload) {
        return (this.getNumber(payload, 'amount') ??
    ...
    resolveCurrency(payload) {
        const currency = this.getString(payload, 'currency') ??
...
exports.NormalizeInfinitePayWebhookService = NormalizeInfinitePayWebhookService;
exports.NormalizeInfinitePayWebhookService = NormalizeInfinitePayWebhookService = __decorate([
    (0, common_1.Injectable)()
], NormalizeInfinitePayWebhookService);
//# sourceMappingURL=normalize-infinitepay-webhook.service.js.map