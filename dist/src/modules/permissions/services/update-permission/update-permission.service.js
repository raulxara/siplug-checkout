...
const update_permission_dto_out_1 = require("./dtos/update-permission.dto-out");
let UpdatePermissionService = class UpdatePermissionService {
    repository;
    ...
    async exec(dtoIn) {
        try {
            ...
            return new update_permission_dto_out_1.UpdatePermissionDtoOut(row);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : 'error on update permission';
...
exports.UpdatePermissionService = UpdatePermissionService = __decorate([
