import { Module } from '@nestjs/common';
import { UseCaseSupportModule } from '../../common/services/use-case-support/use-case-support.module';
import { ClientsModule } from '../../modules/clients/clients.module';
import { OfficesModule } from '../../modules/offices/offices.module';
import { PositionsModule } from '../../modules/positions/positions.module';
import { ProfilesModule } from '../../modules/profiles/profiles.module';
import { SecurityModule } from '../../modules/security/security.module';
import { UserCustomersModule } from '../../modules/user-customers/user-customers.module';
import { UserPositionsModule } from '../../modules/user-positions/user-positions.module';
import { GetAllUsersByOfficeIdController } from './get-all-users-by-office-id.controller';
import { GetAllUsersByOfficeIdUseCase } from './get-all-users-by-office-id.use-case';

@Module({
  imports: [
    OfficesModule,
    ClientsModule,
    UserCustomersModule,
    ProfilesModule,
    UserPositionsModule,
    PositionsModule,
    SecurityModule,
    UseCaseSupportModule,
  ],
  controllers: [GetAllUsersByOfficeIdController],
  providers: [GetAllUsersByOfficeIdUseCase],
  exports: [GetAllUsersByOfficeIdUseCase],
})
export class GetAllUsersByOfficeIdModule {}