...
type MercadoPagoNativeSplitData = {
  enabled: boolean;
  marketplaceId: string | null;
  paymentSplitId: string | null;
  marketplaceFeeAmountInCents: number | null;
...
@Injectable()
export class MercadoPagoGatewayPaymentProvider implements IGatewayPaymentProvider {
  ...
  async processPayment(
    ...
    try {
      ...
      const credentialValidationFailure =
        this.validateMercadoPagoCredentialForTestScenario(dtoIn, accessToken);
  ...
  private async processPaymentLink(params: {
    ...
    const requestPayload = this.buildPaymentLinkPreferenceRequestPayload(
      params.dtoIn,
    );
  ...
  private applyMercadoPagoNativeSplitToPaymentPayload(params: {
    ...
    const applicationFeeAmountInCents =
    ...
    if (
      ...
      throw new Error(
        'Mercado Pago native split requires applicationFeeAmountInCents for /v1/payments',
      );
    }
    ...
    params.payload.application_fee = this.convertCentsToAmount(
    ...
    params.payload.metadata = {
      ...
      applicationFeeAmountInCents,
      applicationFeeAmount: this.convertCentsToAmount(
        applicationFeeAmountInCents,
      ),
    };
  }

  private applyMercadoPagoNativeSplitToPreferencePayload(params: {
    dtoIn: GatewayPaymentDtoIn;
    ...
    const marketplaceFeeAmountInCents =
      splitData.marketplaceFeeAmountInCents ??
      splitData.applicationFeeAmountInCents;
    ...
    if (
      ...
      throw new Error(
        'Mercado Pago native split requires marketplaceFeeAmountInCents for /checkout/preferences',
      );
    }
    ...
    params.payload.marketplace_fee = this.convertCentsToAmount(
    ...
    params.payload.metadata = {
      ...
      paymentSplitId: splitData.paymentSplitId,
      marketplaceFeeAmountInCents,
      marketplaceFeeAmount: this.convertCentsToAmount(
        marketplaceFeeAmountInCents,
      ),
    };
  }
  ...
  private resolveMercadoPagoNativeSplitData(
    ...
    const providerSplit =
      this.asObject(providerPayload.mercadoPagoSplit) ??
    ...
    const transactionSplit =
      this.asObject(transactionConfig.mercadoPagoSplit) ??
    ...
    const gatewaySplit =
      this.asObject(gatewayConfig.mercadoPagoSplit) ??
    ...
    const credentialSplit =
      this.asObject(apiCredentialConfig.mercadoPagoSplit) ??
    ...
    const enabled =
      dtoIn.paymentTransaction.hasSplit === true ||
      dtoIn.paymentTransaction.splitRequired === true ||
      splitObjects.some(
        (item) => this.extractBoolean(item, 'enabled') === true,
      ) ||
      splitObjects.some(
        (item) => this.extractBoolean(item, 'required') === true,
      );
    ...
    return {
      ...
      paymentSplitId:
        this.firstStringFromObjects(splitObjects, [
          'paymentSplitId',
          'payment_split_id',
        ]) ??
  ...
  private assertMercadoPagoFeeAmountIsValid(params: {
    feeAmountInCents: number;
    transactionAmountInCents: number;
    field: string;
  }): void {
    if (
      !Number.isInteger(params.feeAmountInCents) ||
  ...
  private firstStringFromObjects(
  ...
  private firstIntegerFromObjects(
    objects: Array<Record<string, unknown>>,
    keys: string[],
  ): number | null {
    for (const object of objects) {
  ...
  private resolveAccessToken(dtoIn: GatewayPaymentDtoIn): string {
    ...
    const shouldPreferTestToken =
      this.isSandboxEnvironment(dtoIn) ||
  ...
  private resolveDateOfExpiration(dtoIn: GatewayPaymentDtoIn): string | null {
    ...
    const normalizedConfiguredDate =
      this.normalizeMercadoPagoDateOfExpiration(configuredDate);
  ...
  private buildFutureMercadoPagoExpirationDate(daysFromNow: number): string {
    const date = new Date();
  ...
  private validateMercadoPagoCredentialForTestScenario(
    ...
    return new GatewayPaymentDtoOut({
      ...
      processMessage:
        'Mercado Pago TEST access token is required for sandbox/test PIX, boleto and credit_card payments',
      ...
      providerResponse: {
        message:
          'Current access token looks like a production/live credential, but this is a Mercado Pago test scenario. Use a TEST access token generated for the seller test account and generate cardToken with the matching TEST public key.',
      },
  ...
  private isMercadoPagoLiveAccessToken(accessToken: string): boolean {
    const normalized = accessToken.trim();

    return (
      normalized.startsWith('APP_USR-') || normalized.startsWith('APP_USR')
    );
  }

  private isMercadoPagoTestPayer(dtoIn: GatewayPaymentDtoIn): boolean {
    const providerPayload = dtoIn.providerPayload ?? {};
  ...
  private isMercadoPagoTestCardPayload(dtoIn: GatewayPaymentDtoIn): boolean {
    const providerPayload = dtoIn.providerPayload ?? {};
    const paymentData = this.asObject(providerPayload.paymentData);
  ...
  private resolveMercadoPagoEnvironment(dtoIn: GatewayPaymentDtoIn): string {
    const config = dtoIn.config ?? {};
    const transactionConfig = this.asObject(config.transactionConfig);
    const gatewayConfig = this.asObject(config.gatewayConfig);
    const apiCredentialConfig = this.asObject(config.apiCredentialConfig);
    ...
    return (
      this.toNullableString(transactionConfig.environment) ??
  ...
  private isSandboxEnvironment(dtoIn: GatewayPaymentDtoIn): boolean {
  ...
  private resolvePreferenceCheckoutUrl(params: {
    dtoIn: GatewayPaymentDtoIn;
    responseBody: MercadoPagoPreferenceResponse;
  }): string | null {
