...
@Injectable()
export class StripeRecurringPaymentProvider {
  async createSubscription(
    ...
    const responseBody = await this.parseJsonResponse(
      response,
      'Stripe returned a non JSON recurring subscription response',
    );
    ...
    if (!response.ok) {
      return new GatewayRecurringPaymentDtoOut(
        ...
        null,
        this.resolveStripePriceId(dtoIn),
        null,
        null,
        ...
        this.resolveStripeErrorMessage(responseBody, response.status),

        this.sanitizePayload(this.urlSearchParamsToObject(requestPayload)),
    ...
    const paymentStatus =
      this.toNullableString(responseBody.payment_status) ?? 'unpaid';
  ...
  private buildCheckoutSessionRequest(
    ...
    const checkoutSession = this.asObject(
      dtoIn.providerPayload.checkoutSession,
    );

    const paymentMethodType = this.resolveStripePaymentMethodType(
      dtoIn.paymentTransaction.paymentMethod,
    );

    if (paymentMethodType === null) {
      throw new Error(
        `Stripe recurring payment does not support paymentMethod: ${dtoIn.paymentTransaction.paymentMethod}`,
      );
    }
    ...
    const customerEmail =
      this.toNullableString(payer.email) ??
    ...
    if (stripePriceId !== null) {
      ...
      params.append(
        ...
        dtoIn.paymentTransaction.currency.toLowerCase(),
      );

      params.append(
        'line_items[0][price_data][unit_amount]',
        String(dtoIn.paymentTransaction.amount),
      );

      params.append(
        'line_items[0][price_data][recurring][interval]',
        this.mapStripeInterval(dtoIn.subscriptionPlan.billingInterval),
      );

      params.append(
        'line_items[0][price_data][recurring][interval_count]',
        String(dtoIn.subscriptionPlan.billingIntervalCount),
      );

      params.append(
        'line_items[0][price_data][product_data][name]',
        this.limitText(dtoIn.subscriptionPlan.name, 250),
      );

      if (dtoIn.subscriptionPlan.description !== null) {
        params.append(
          'line_items[0][price_data][product_data][description]',
          this.limitText(dtoIn.subscriptionPlan.description, 500),
        );
      }
    ...
    if (
      ...
      params.append(
        ...
        String(dtoIn.subscriptionPlan.trialDays),
      );
    }
    ...
    params.append(
      'subscription_data[metadata][checkoutSessionId]',
      this.toNullableString(checkoutSession._id) ?? '',
    );

    params.append(
      'subscription_data[metadata][paymentTransactionId]',
    ...
    params.append(
      'subscription_data[metadata][subscriptionPlanId]',
      dtoIn.subscriptionPlan._id,
    );

    params.append(
      'subscription_data[metadata][subscriptionInvoiceId]',
      dtoIn.subscriptionInvoice._id,
    );
  ...
  private resolveStripePriceId(dtoIn: GatewayRecurringPaymentDtoIn): string | null {
    ...
    return (
      this.toNullableString(stripeMapping.priceId) ??
  ...
  private mapStripeInterval(interval: string): string {
    if (interval === 'day') {
      return 'day';
    }
  ...
  private mapStripeCheckoutStatus(params: {
    status: string;
    ...
    if (status === 'complete' || paymentStatus === 'paid') {
      return {
        ...
        processMessage:
          'Stripe Checkout subscription completed and waiting webhook confirmation',
      };
    }
    ...
    return {
      ...
      processMessage:
        'Stripe Checkout subscription session created and waiting buyer approval',
    };
  }
  ...
  private normalizeStripeBaseUrl(baseUrl: string): string {
  ...
  private resolveCancelUrl(dtoIn: GatewayRecurringPaymentDtoIn): string | null {
    ...
    return (
      this.toNullableString(checkoutSessionConfig.cancelUrl) ??
  ...
  private ensureStripeSessionPlaceholder(url: string): string {
  ...
  private resolveStripeErrorMessage(
    ...
    const message =
      this.toNullableString(error.message) ??
